...
class Calculator:
    def __init__(self, master):
        self.master = master
        master.title("Skaiciuotuvas")

        # Sukuriam nauja teksto lauka, kuriame bus rodomas skaicius
    ...
    def create_button(self, text, row, column, columnspan=1):
        button = tk.Button(self.master, text=text, width=7, height=2, font=("Arial", 14),
    ...
    def button_click(self, text):
        if text == "C":
            ...
            try:
                ...
                self.display.delete(0, tk.END)
                self.display.insert(0, "Klaida")
        else:
            self.display.insert(tk.END, text)

# Sukuriam pagrindini langa
root = tk.Tk()
